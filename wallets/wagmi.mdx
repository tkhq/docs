...
**Wagmi Connector:** A custom connector built using Wagmi's `createConnector` utility. It wraps our EIP-1193 provider, making the wallet compatible with the Wagmi ecosystem. Reference the `popup-wallet-demo`'s `@/apps/dapp/lib/connector.ts` and `@/apps/dapp/lib/wagmi.ts` provide concrete examples.
...
sequenceDiagram
    ...
    Note over EmbeddedWalletPopup,TurnkeyAPI: User Authenticates & Selects Account
...
<video
  ...
  controls
  muted
  className="rounded-md my-4 w-full max-w-2xl mx-auto"
/>
