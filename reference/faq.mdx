...
<AccordionGroup>
  ...
  <Accordion title="Do you have any rate limits in place in your public API?">
    ...
    When rate limits are exceeded, an error with HTTP 429 is returned with the following message: `Too many requests in a short period of time. Please wait a few minutes and try again`.

    Rate limits are enforced **per organization**, not per IP address. Requests from all of your servers count toward the same limit, so distributing traffic across multiple machines or IP addresses will not increase your available throughput.
