...
1. **Stake TRX for Bandwidth (resource)**
   1. Ensure that your organization’s Turnkey wallet is funded with a sufficient TRX balance
      - **TIP:** This can be a wallet contained within your parent organization
   2. Initialize the staking activity by calling the [<u>freezeBalanceV2</u>](https://developers.tron.network/reference/freezebalancev2-1) function
...
2. **Delegate the resource to end-user wallets**
   1. After you’ve initialized the staking operation using freezeBalanceV2 (Step 2B) you’re now able to delegate the resources to end-user wallets
   2. From the staking wallet, make a call to the Tron network using [<u>DelegateResource</u>](https://developers.tron.network/reference/delegateresource-1) function
...
3. **Transaction flow (end-user)**
   1. After the staking has been initialized and resources delegated, the user experience is trivial: end-user wallets (receiver_address) can transact via the Tron network without needing to provide gas for their own transactions.
