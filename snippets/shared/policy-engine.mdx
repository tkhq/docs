...
Every policy also carries a `policyName` and a `notes` field. Both are required by the API when creating a policy; `notes` may be an empty string.

#### See below for an example policy that allows a single user to send transactions to a single address

```json
{
  "policyName": "Allow a single user to send transactions to <ALLOWED_ADDRESS>",
  "notes": "",
  "effect": "EFFECT_ALLOW",
...
- The Root Quorum bypasses any policies.
