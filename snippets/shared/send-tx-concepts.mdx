...
<Note>
  ...
  Rent` is disabled by default and must be enabled in the dashboard before Turnkey will pre-fund rent for account creation. If created accounts are later closed, refunded rent can go back to the signer rather than the sponsor. See [Solana Rent Sponsorship](/networks/solana-rent-refunds). For payer behavior, static-key requirements, and account-creation caveats in sponsored flows, see [Solana transaction construction for sponsored flows](/networks/solana-transaction-construction).
</Note>
