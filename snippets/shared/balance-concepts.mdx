...
<Note>
  ...
  still occur and the transaction may be removed from the canonical chain. Subscribe to
  `BALANCE_FINALIZED_UPDATES` to receive events after the block has reached the finalization
  threshold.
</Note>
