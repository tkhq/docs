...
<Note>
  Balance webhooks fire only for assets in the [supported asset list](/api-reference/queries/list-supported-assets). Internal calls and traces (e.g. value moved between contracts within a single transaction) are not parsed and will not trigger a webhook. Support for internal calls and traces will be added before this feature exits closed beta.
</Note>
