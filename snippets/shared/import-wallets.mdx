...
<Warning>
  **Breaking change:** The import iframe now returns an `ERROR` when `organizationId` or `userId` are missing from the injected import bundle. Clients using `@turnkey/iframe-stamper` **< 2.0.0** (or custom integrations that omit these fields) must upgrade to **>= 2.0.0** (latest: 2.11.0).
</Warning>
