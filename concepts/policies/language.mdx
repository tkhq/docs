...
|                          | from                     | string (case insensitive)                 | The signer address of the transaction                                                                                                                                                                                                                                                  |
|                          | nonce                    | int                                       | The nonce for the transaction                                                                                                                                                                                                                                                          |
...
|                          | fee_token                | string (case insensitive)                 | The token address used to pay transaction fees (empty string when paying with the native token)                                                                                                                                                                                        |
...
|                                 | address_type            | string (case insensitive)             | The address derivation type of the address for this transaction output                                                                                                                                                                                     |
...
| **TempoCall**                   | to                      | string (case insensitive)             | The destination address of the call (empty string for contract creation)                                                                                                                                                                                   |
|                                 | value                   | int                                   | The value carried by this call (in wei)                                                                                                                                                                                                                    |
|                                 | input                   | string (case insensitive)             | The raw call data as a hex string (supports slicing, e.g. `tempo.tx.calls[0].input[34..74]`)                                                                                                                                                              |
...
|                              | CREATE |                       ACTIVITY_TYPE_OTP_LOGIN |
...
<Note> The policy engine's `int` type is limited to 128 bits (i128). Ethereum smart contracts support signed integers up to 256 bits (`int256`), but values exceeding the 128-bit signed range cannot be used in policy conditions. </Note>
...
Our Tempo policy language (accessible via `tempo.tx`) allows for policy control over signing Tempo transactions. Tempo transactions natively support batched calls — a single transaction can contain one or more calls that execute atomically. The `tempo.tx.calls` list gives you access to each individual call, including its destination, value, and raw input data.

The `tempo.tx` namespace exposes the following transaction-level fields: `chain_id`, `from`, `nonce`, `gas_limit`, `max_fee_per_gas`, `max_priority_fee_per_gas`, `fee_token`, `valid_before`, and `valid_after`. Each call in `tempo.tx.calls` exposes: `to`, `value`, `input`, and `function_signature`.
...
<Note>
  Tempo does not currently support [Smart Contract Interfaces](/concepts/policies/smart-contract-interfaces) (ABI parsing). To inspect encoded arguments within call data, you can use slicing on the `input` field (e.g. `tempo.tx.calls[0].input[34..74]` to extract the first ABI-encoded address argument). The `input` field is case-insensitive, so hex comparisons work regardless of casing.
</Note>

See the [Tempo policy examples](/concepts/policies/examples/tempo) for sample scenarios.

### Built-in: `IsGuaranteedWalletAccountInWallet`

`IsGuaranteedWalletAccountInWallet(account_address, wallet_id)` returns a boolean indicating whether the provided wallet account address is guaranteed to belong to the specified wallet.

- `account_address`: the wallet account address as a hex string.
- `wallet_id`: the wallet identifier.
- Hex string comparisons are case-insensitive.
- The function returns `true` only when membership is guaranteed, and `false` otherwise.

This built-in can be used in any policy condition — for example, to verify that a call destination or an ABI-encoded recipient address belongs to a specific wallet. See the [Tempo policy examples](/concepts/policies/examples/tempo#raw-calldata-inspection) for usage with calldata slicing.
