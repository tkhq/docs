...
|                          | fee_token                | string                                    | The token address used to pay transaction fees (empty string when paying with the native token)                                                                                                                                                                                        |
...
<Note>
  ...
  Interfaces](/concepts/policies/smart-contract-interfaces) (ABI parsing). To
  inspect encoded arguments within call data, you can use slicing on the `input`
  field (e.g. `tempo.tx.calls[0].input[34..74]` to extract the first ABI-encoded
  address argument). The `input` field is case-insensitive, so hex comparisons
