---
title: "Co-signing"
description: "With Turnkey, you can configure a bespoke co-signing architecture based on your needs."
sidebarTitle: "Co-signing"
---

## Overview

There are many scenarios where developers may want to host a backend for co-signing purposes. For example, if you would like to perform operations after an activity is submitted, approved, or completed, you could use a separate backend to receive [webhook events](/developer-reference/webhooks.mdx) from Turnkey and respond appropriately.
...
const turnkeyClient = new Turnkey({
  ...
  apiPublicKey: process.env.API_PUBLIC_KEY!, // co-signer-specific API key
  apiPrivateKey: process.env.API_PRIVATE_KEY!,
  defaultOrganizationId: process.env.ORGANIZATION_ID!, // parent organization
});

// Your server needs to handle post requests sent from Turnkey
// at the endpoint of your choosing.
app.post('/turnkey-webhook-endpoint', (req, res) => {
  const { activity } = req.body;

  // Unique identifier for a Turnkey activity
  const { fingerprint } = activity;

  // Perform balance checks, database checks, or other operations

  ...

  // If all looks good, approve the activity using the co-signer's API key
  const approvalResponse = await turnkeyClient.apiClient().approveActivity({
    fingerprint,
  });
...
<Accordion title="When does the user 'approve' a delegated access setup?">
  End-user approval typically happens during **<u>intent</u>** creation — for
  example, when the end-user authorizes a DA user by adjusting the quorum or
  signs an initial setup transaction.
</Accordion>
...
<Accordion title="If a delegated API key is leaked, does that allow someone to act on behalf of the user?">
  Yes — if the key is attached to a broad policy. That’s why it’s important to
  limit the scope of policies and enforce API hygiene practices.
</Accordion>

<Accordion title="Is this the same risk as having a master delegate account?">
  In effect, yes. The key difference is that granular policies **can restrict**
  what a DA user can do, offering better security hygiene even if there's still
  elevated access.
</Accordion>
...
<Accordion title="Can I dynamically add policies per limit order without user friction?">
  Yes — this is a common pattern. You can add a policy per order (limit, stop
...
<Accordion title="Are time-bound transactions supported?">
  Yes, on Solana via `solana.tx.recent_blockhash`, which restricts a
  transaction’s validity to a ~60–90 second window. Not ideal for delayed
  executions (e.g. limit orders), but useful for immediate, single-use actions.
</Accordion>
...
<Accordion title="What policy strategies work well for Solana limit orders?">
  Use structure-based conditions (instruction count, token recipient), and if
  possible, include conditions like `recent_blockhash` for ephemeral actions.
  For limit orders with unknown execution time, dynamic policy creation per
  order is safer than relying on stale conditions.
</Accordion>
