#!/usr/bin/env python3
"""
Step 2: Update docs.json navigation paths based on Phase 3 URL mapping.

This script reads docs.json, walks the "navigation" section recursively,
and replaces old paths with new paths according to the Phase 3 mapping table.
Only the "navigation" section is modified; redirects and everything else
are left untouched.

Usage:
    python step2-update-docsjson.py
"""
...
# Paths use the docs.json convention: no leading slash, no .mdx extension.
# ---------------------------------------------------------------------------
...
def walk_and_replace(node, mapping: dict[str, str]) -> int:
    """
    Recursively walk a JSON structure (the navigation section).
    For every string found inside a "pages" array, check if it matches
    an old path in the mapping. If so, replace it with the new path.
    ...
    if isinstance(node, dict):
        for key, value in node.items():
            if key == "pages" and isinstance(value, list):
                # Walk the pages array
                for i, item in enumerate(value):
                    if isinstance(item, str):
                        if item in mapping:
                            node[key][i] = mapping[item]
                            replacements += 1
                    elif isinstance(item, dict):
                        replacements += walk_and_replace(item, mapping)
            else:
                replacements += walk_and_replace(value, mapping)
...
def main() -> None:
    ...
    # --- Extract and process only the navigation section ---
    if "navigation" not in data:
        print("ERROR: No 'navigation' key found in docs.json.", file=sys.stderr)
        sys.exit(1)

    # Track which old paths we actually replaced (to find missing ones later)
    tracking_mapping = dict(PATH_MAPPING)  # copy we can pop from

    navigation = data["navigation"]
    replacements = walk_and_replace(navigation, PATH_MAPPING)

    # --- Figure out which old paths were NOT found in navigation ---
    # Re-walk the updated navigation to collect all page strings
    def collect_all_pages(node: object) -> set[str]:
        pages: set[str] = set()
        if isinstance(node, dict):
            for key, value in node.items():
                if key == "pages" and isinstance(value, list):
                    for item in value:
                        if isinstance(item, str):
                            ...
                            pages.update(collect_all_pages(item))
                else:
                    pages.update(collect_all_pages(value))
        elif isinstance(node, list):
            for item in node:
                pages.update(collect_all_pages(item))
        return pages

    all_new_pages = collect_all_pages(navigation)

    # An old path is "not found" if its NEW value does not appear in the
    # final navigation (meaning the old path was never encountered).
    not_found: list[str] = []
    for old_path, new_path in PATH_MAPPING.items():
        if new_path not in all_new_pages:
    ...
    print(f"Old paths NOT found in docs.json navigation: {len(not_found)}")
    if not_found:
        print("\nWARNING: The following old paths were not found in navigation:")
        for p in sorted(not_found):
            print(f"  - {p}")
    else:
        print("All old paths were found and replaced successfully.")
