...
<Frame>
  <img
    src="/images/authentication/img/oauth_signup.png"
...
<Frame>
  <img
    src="/images/authentication/img/oauth_login.png"
...
1. The end-user authenticates with Google, their email address can be automatically linked to their authentication methods and considered "verified", allowing for email otp or email auth login in the future.
2. The end-user authenticates with a Google email (@gmail.com) via email otp or email auth. If this user tries to authenticate with Google later, Google OIDC will be automatically added as a valid login method for that user provided that the email in the Google account matches the aforementioned Google email.
3. The end-user has existing non-Google authentication methods (e.g. phone number, passkeys, etc.) and later attempts to attempts to add Google OIDC as a login method (via [CREATE_OAUTH_PROVIDERS](/api-reference/activities/create-oauth-providers#api-key)). The email address in the Google account will be automatically marked as "verified" and linked to the existing user.
