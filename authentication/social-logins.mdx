...
2) **OAuth-Only Providers**
(X/Twitter, Discord…)
These Providers do *not* issue **OIDC ID Tokens** themselves, and instead provide only bare-bones OAuth 2.0.  To work around this limitation, Turnkey runs the **OAuth 2.0 Authorization Code + PKCE** flow, calls the provider’s `who-am-I` endpoint using your app credentials, and then uses the returned User Data to **mint a short-lived OIDC ID token**.  In effect, Turnkey acts as an "*OIDC Wrapper*" for these OAuth 2.0 Services.
...
| **Provider Type**   | **OIDC Provider**                                 | **OAuth-Only Providers**                                          |
|---------------------|---------------------------------------------------|-------------------------------------------------------------------|
| Providers           | Google, Apple, Facebook, Auth0, Cognito           | X/Twitter, Discord                                                |
| Token Transport     | Returned to your App by Provider                  | Returned by Turnkey after **OAuth 2.0 Auth Code + PKCE** flow     |
...
- **Turnkey**: Runs verifiable infrastructure to create credentials, verify OIDC tokens and, in the case of **OAuth-Only Providers**, issue them as well.
...
sequenceDiagram
    ...
    OAuth Provider->>User: Prompt to grant permission
    User->>OAuth Provider: Grants permission
    OAuth Provider->>Backend: Return auth_code via redirect_uri
    Backend->>Turnkey: Send auth_code + redirect_uri
...
<Steps>
  ...
  The backend exchanges sends the auth_code + redirect_uri to Turnkey in exchange for an OIDC ID Token.
  </Step>
...
## Setup for OAuth-Only Providers
