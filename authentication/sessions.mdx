...
If you’d like to do this via our SDK abstractions, you can use the [httpClient](https://docs.turnkey.com/sdks/react/advanced-api-requests#the-http-client) provided by the `useTurnkey` hook and call the [createReadOnlySession](https://github.com/tkhq/sdk/blob/56ac61755db2aa7b31fd0edf125635dd88cf98a1/packages/core/src/__generated__/sdk-client-base.ts#L2655) method, which creates a `CreateReadOnlySession` activity under the hood. 
You can use the [storeSession](https://docs.turnkey.com/sdks/react/advanced-backend-authentication#storing-the-session) function from the `useTurnkey` hook to store the session token, the SDK will automatically handle the session and keypair management for you.
