...
### Email Templates and Custom Domains

We also support custom HTML email templates and sender email addresses (`sendFromEmailAddress`) for [Enterprise](https://www.turnkey.com/pricing) clients on the **Scale** tier. This allows you to inject arbitrary data from a JSON string containing key-value pairs. In this case, the `emailCustomization` variable may look like:
