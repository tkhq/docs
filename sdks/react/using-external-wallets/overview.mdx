...
Here’s an example of how MetaMask `WalletProvider` objects are returned when both Ethereum and Solana native support are enabled. MetaMask appears as two separate providers, one for each namespace.  
