...
**New Approach (Hook-Based: `useTurnkey()` + Proxy)**

The provider handles proxy calls; you no longer need to stamp server-side with the parent org key, nor manually exchange verification tokens. Client code initializes OTP via the proxy and completes it with `completeOtp`.
...
Use `useTurnkey()` helpers `createWallet`, `createWalletAccounts`, and `refreshWallets`. The provider manages session and state; you only invoke the actions and refresh.
...
export function WalletActions() {
  const { createWallet, createWalletAccounts, refreshWallets, wallets } =
    useTurnkey();
...
Use `useTurnkey()`'s `signAndSendTransaction` helper. The provider handles session state and request stamping; you pass a wallet account and an unsigned transaction.
For more details, see: [/sdks/react/signing#signing-transactions](/sdks/react/signing#signing-transactions).
