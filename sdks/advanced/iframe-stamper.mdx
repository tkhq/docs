...
<Warning>
  **Security update — upgrade required for import flows.** A recent hardening of Turnkey's iframe frames enforces stricter cross-origin validation. As part of this, the import iframe now returns an `ERROR` (instead of silently continuing) when `organizationId` or `userId` are missing from the injected import bundle. Clients on `@turnkey/iframe-stamper` **< 2.0.0** are affected. **Upgrade to `@turnkey/iframe-stamper` >= 2.0.0** (latest: 2.11.0) to avoid breakage. Custom integrations must also include `organizationId` and `userId` in injected import bundles.
</Warning>
