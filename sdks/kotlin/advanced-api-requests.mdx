...
However, you can also make advanced API requests directly to Turnkey's endpoints if you need more control or want to implement custom features in your Kotlin app.

## The HTTP client

To make advanced API requests, use the `client` from the `TurnkeyContext`. This client is tied to the active session, so stamping and organization context are automatically handled for you.
