...
description: "Learn how to create and manage embedded wallets in your Kotlin application using Turnkey's Kotlin SDK."
sidebarTitle: "Using embedded wallets"
---

## Overview

The Kotlin SDK allows you to create and manage embedded wallets in your Kotlin application. This includes creating wallets, deriving wallet accounts, and accessing wallet information.
...
lifecycleScope.launch {
    ...
    val selectedWallet = TurnkeyContext.wallets.value?.firstOrNull() ?: return@launch
...
lifecycleScope.launch {
    ...
    val (targetPublicKey, _, _) = generateP256KeyPair() // Import from turnkey-crypto
...
lifecycleScope.launch {
    ...
    val wallets = TurnkeyContext.wallets.value.orEmpty()
    val selectedWallet = wallets.getOrNull(0) ?: return@launch
    val selectedAccount = selectedWallet.accounts.getOrNull(0) ?: return@launch
...
lifecycleScope.launch {
    try {
        val m = mnemonic.value // Your user's mnemonic string from StateFlow
