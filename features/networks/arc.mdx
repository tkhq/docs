...
- **USDC is the native gas token.** ETH is not the native asset on Arc — gas is paid in USDC, so accounts need a USDC balance to transact. Native USDC uses an 18-decimal interface (standard `msg.value` semantics), while the ERC-20 interface on the same balance uses 6 decimals. To display a native value as USDC, divide by 10<sup>12</sup>. Don't credit or record balances from the 6-decimal value: truncation at the 6-decimal boundary records less than was transferred.
...
Testnet funds are available from the [Circle faucet](https://faucet.circle.com), and transactions can be tracked on the [Arc testnet explorer](https://testnet.arcscan.app).
