...
For sponsored sends, Turnkey uses the request-level `recentBlockhash` if supplied, or fetches a fresh blockhash during execution.
For legacy/V0 transactions, Turnkey preserves explicit compute unit limits and prices in Compute Budget instructions and fills missing values.
...
Set `sponsor: false` to have the transaction's fee payer pay the network fees. Turnkey signs and broadcasts the submitted message without changing its blockhash or compute and fee controls. Supply a current blockhash and the required compute and fee values in the serialized transaction.
