...
Turnkey can fill missing blockhashes for customer-paid sends and select blockhashes for sponsored sends.
...
Turnkey broadcasts the transaction and monitors its status for both sponsored and customer-paid sends.
...
Turnkey preserves the submitted instructions and compute and fee controls.
It preserves a supplied blockhash or fills the all-zero placeholder before policy evaluation and signing.
[Blockhash auto-fill](/features/networks/solana-transactions#customer-paid-sends) requires an unsigned transaction and all required signers.
