...
For V1 wire encoding and outer transaction config, see [Solana V1 transactions](/features/networks/solana-v1).
...
- **Compute and fee controls**: for legacy/V0, Turnkey preserves explicit compute unit limits and prices in Compute Budget instructions and fills missing values. V1 uses [outer transaction config](/features/networks/solana-v1#v1-config-during-sponsorship); Compute Budget instructions are no-ops for V1.
...
See the [V1 limits](/features/networks/solana-v1#wire-encoding-and-limits) before submitting a prebuilt transaction.
