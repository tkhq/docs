...
With `sponsor: false`, Turnkey preserves the submitted instructions and compute and fee controls.
It preserves a supplied blockhash or fills a missing one before policy evaluation and signing.
Supply the compute and fee controls your application requires in the serialized transaction.
...
For standalone `signTransaction` requests, supply the blockhash in the serialized transaction.

### Customer-paid sends

For `sponsor: false`, Turnkey selects the blockhash as follows:

| Blockhash in `unsignedTransaction` | Request-level `recentBlockhash` | Behavior |
| --- | --- | --- |
| Nonzero | Omitted or matching | Preserves the embedded blockhash |
...
The serialized transaction must still contain this field; do not remove its bytes.
The request-level `recentBlockhash`, if provided, must be a nonzero blockhash.
...
Turnkey preserves a nonzero embedded blockhash even if it is stale; auto-fill does not refresh expired blockhashes.

### Sponsored sends

For `sponsor: true`, Turnkey uses the request-level `recentBlockhash` if supplied.
...
For customer-paid sends with no supplied blockhash, each access-control evaluation can select a new one.
The request's original intent stays the same, but the message selected for signing can change.
