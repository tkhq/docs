...
Note: several examples below scope a policy with `wallet_account.address`, which is matched exactly, including case. Substitute `<SIGNER_ADDRESS>` with the address exactly as the API returns it (the EIP-55 checksummed, mixed-case form) rather than lowercasing it. See [Case sensitivity](/features/policies/language#case-sensitivity).
