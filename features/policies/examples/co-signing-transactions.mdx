...
<Note>
  See the [with-cosigning example](https://github.com/tkhq/sdk/tree/main/examples/access-control/with-cosigning) for a full working implementation using Next.js, OTP login, SSE webhooks, and a 2-of-2 root quorum.
</Note>
...
<Note>
  The quorum is symmetric — the backend can also initiate signing (vote 1) and the user approves (vote 2). See the [with-cosigning example](https://github.com/tkhq/sdk/tree/main/examples/access-control/with-cosigning) for a full walkthrough of both flows.
</Note>
