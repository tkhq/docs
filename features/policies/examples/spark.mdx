...
Spark wallet accounts separate key roles by derivation path, under coin type `8797555'`:

- Identity key: `m/8797555'/<account>'/0'`
- Signing (leaf) keys: `m/8797555'/<account>'/1'/<index>'`
- Static deposit keys: `m/8797555'/<account>'/3'/<index>'`

The static deposit flow — generating a static deposit address and claiming an incoming L1 Bitcoin
payment — exports the static deposit key with `EXPORT_WALLET_ACCOUNT`. Use the
...
This policy matches only fully hardened four-component paths with coin type `8797555` and purpose
...
```json
{
  "policyName": "Allow exports of Spark static deposit keys only",
  "effect": "EFFECT_ALLOW",
  "consensus": "approvers.any(user, user.id == '<DELEGATED_USER_ID>')",
...
<Note>
  The `wallet_account.path_indexes` and `wallet_account.path_hardened` fields are set only for
  `EXPORT_WALLET_ACCOUNT` activities. For more export policies, see
  [Access control](/features/policies/examples/access-control#allow-exporting-only-wallet-accounts-under-a-specific-derivation-subtree).
</Note>
