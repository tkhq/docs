...
This pattern can authorize leaf swaps with known Spark Service Providers (SSPs) without allowing
the delegated user to prepare a transfer to an arbitrary receiver.
...
The Spark activity parameters available today do not expose every business-level detail of a Spark
operation:

- `SPARK_PREPARE_TRANSFER` does not expose a transfer amount or an operation type that distinguishes
  a leaf swap from an external send. A receiver allowlist can identify trusted SSP receivers when
  that distinction is sufficient.
- `SparkKeyDerivation.type` identifies the cryptographic key variant, not the business operation.
  It cannot distinguish two operations that both sign with a `signing_leaf` derivation.
- `EXPORT_WALLET_ACCOUNT` does not expose the Spark derivation type or derivation path. The
  `static_deposit` FROST policy above governs `SPARK_SIGN_FROST`; it does not restrict wallet-account
  exports to the static-deposit path.
- Spark token operations use `SIGN_RAW_PAYLOAD`. Spark token ID, amount, recipient, and validity
  fields are not parsed into `activity.params`, so the policies on this page cannot enforce those
  limits.
