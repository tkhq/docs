...
You can restrict wallet account exports by the account's BIP32 derivation
path. Use `wallet_account.path_indexes` (the path indexes, with the hardening
bit stripped) and `wallet_account.path_hardened` (whether each component is
hardened). This policy lets a user export only accounts in the fully hardened
subtree `m/8797555'/*'/3'/*'`:

```json JSON
{
  "policyName": "Allow exporting only accounts under m/8797555'/*'/3'/*'",
  "effect": "EFFECT_ALLOW",
  "consensus": "approvers.any(user, user.id == '<USER_ID>')",
...
For a concrete use of this pattern, see
[Spark key exports](/features/policies/examples/spark#key-exports): the same
condition limits exports to Spark static deposit keys.

<Note>
  Derivation paths contain apostrophes (e.g. `m/44'/60'/0'/0/0`). Policy
  string literals cannot contain apostrophes. For this reason, the path is
  exposed as the `path_indexes` and `path_hardened` lists, not as a string.
</Note>

#### Deny exporting wallet accounts by coin type

An explicit deny always wins over allow policies. You can grant a broad export
allow, then deny part of the derivation tree. This policy blocks exports of
accounts whose coin type (the second path component) is 60 (Ethereum):

```json JSON
{
  "policyName": "Deny exporting Ethereum (coin type 60) accounts",
  "effect": "EFFECT_DENY",
  "condition": "activity.kind == 'EXPORT_WALLET_ACCOUNT' && wallet_account.path_indexes[1] == 60"
}
```

<Note>
  ED25519 derivation hardens every path component. Two spellings such as
  `m/44'/501'` and `m/44/501` derive the same key. For ED25519 accounts,
  write path conditions with `path_indexes` only, not `path_hardened`.
</Note>
