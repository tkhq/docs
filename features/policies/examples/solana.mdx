...
It caps the original V1 priority fee at 1,000 total lamports and requires explicit resource limits within the stated bounds.
...
For example, set the original config to 200,000 CU, 67,108,864 loaded-account bytes, 32,768 heap bytes, and a 1,000-lamport priority fee.
Turnkey preserves these explicit config values during sponsorship.
Policies inspect the original instruction list, so sponsor-added rent-funding instructions do not change this policy's instruction count.
