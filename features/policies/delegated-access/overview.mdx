...
- Create an **API-only user (Delegated User)** with an API key authenticator that you control. This key can then be used server-side to sign transactions on the user’s behalf.
