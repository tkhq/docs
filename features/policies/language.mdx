...
<Note>
  ...
  `activity.resource` + `activity.action` to target a broad category. Reach for `activity.type`
  only when you deliberately need one exact version.
</Note>
...
The `activity.params` field exposes specific parameters based on the activity type. Most activities
do not expose any parameters, but certain activities expose fields that can be used in policy
conditions.

Only the fields listed below are available to policies. An activity's API request may contain other
fields that are not exposed by the policy evaluator; attempting to access one of those fields
produces a `MissingField` evaluation error.
