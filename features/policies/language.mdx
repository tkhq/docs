...
for their signatures and constraints; see [Time-based
policies](/features/policies/time-based-policies) for the model behind them and worked examples.
...
opened by the schedule. See [Active time
spans](/features/policies/time-based-policies#active-time-spans-recurring) for the fire-plus-duration
model and its daylight-saving behavior.
...
Day-of-month and day-of-week are **OR'd, not AND'd**: when both fields are restricted, the cron fires
when either matches, and this subset has no modifier to force AND semantics. This can broaden the
window in which a policy is active — see [Active time
spans](/features/policies/time-based-policies#active-time-spans-recurring) for details.
...
the total duration must be **greater than zero and no more than 7 days**.

The `<tz>` argument accepts any IANA time zone name — see the [IANA time zone
