...
<Note>
  {" "}
  The `ContractArgument` type, used in documentation for ABI and IDL arguments
  represents an enum indicating this type could be any one of the string,
  number, array or struct types listed in our Primitives section.{" "}
</Note>
...
<Note>
  ** In the Activity breakdown table above, `**` marks legacy features
  deprecated in the latest SDKs. This note applies only to that table.
</Note>
...
<Note>
  This table covers policy-governed activities only. Activities governed by the
  root quorum (see [Root quorum activities](#root-quorum-activities)) are not
  policy-governed and have no `activity.kind`.
</Note>
...
<Note>
  Prefer `activity.kind` over `activity.type` when you want a policy to apply
  across all versions of an activity. `activity.type` targets one exact version
  and will not match newer versions introduced later.
</Note>
...
<Note>
  Rule of thumb: use `activity.kind` to target a single activity
  (version-agnostic), and `activity.resource` + `activity.action` to target a
  broad category. Reach for `activity.type` only when you deliberately need one
  exact version.
</Note>
...
<Note>
  {" "}
  The policy engine's `int` type is limited to 128 bits (i128). Ethereum smart
  contracts support signed integers up to 256 bits (`int256`), but values
  exceeding the 128-bit signed range cannot be used in policy conditions.{" "}
</Note>
...
Spark optional string fields evaluate to an empty string when omitted. `SparkKeyDerivation`
supports five variants: `identity`, `signing_leaf`, `deposit`, `static_deposit`, and
`htlc_preimage`. Only `signing_leaf` and `static_deposit` have variant-specific data, so those are
the only variants with nested fields (see table below).

At policy runtime, inactive variant fields evaluate to their type defaults rather than producing an evaluation error. For example,
by default `signing_leaf.leaf_id` would an empty string, and `static_deposit.index` would return `0`. These
defaults do *not* indicate that the variant is active. We'd recommend checking `SparkKeyDerivation.type` *before*
targeting a variant-specific field. Note: the `type` field is an empty string when unset, and `0` is a valid
