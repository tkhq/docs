...
Spark optional string fields evaluate to an empty string when omitted. Fields belonging to an
unused key derivation variant evaluate to an empty string or `0`, rather than producing an
evaluation error at runtime. 

Notes:
- Use `SparkKeyDerivation.type` to distinguish variants (e.g. `signing_leaf`, `deposit`, `static_deposit`, or `htlc_preimage`)
- `0` is a valid static deposit index.
