...
Execute only accepts the bound quote terms. You do not pass provider payloads, calldata, or fee overrides. Because none of these inputs exist in the V2 intent, none of them can be tampered with before signing. Optional replay-protection fields (`evmNonce`, `recentBlockhash`, `gasStationNonce`) can be set on the intent; omit them to auto-fetch. See [Trust boundary](/features/transaction-management/swap#trust-boundary) for the full model.
