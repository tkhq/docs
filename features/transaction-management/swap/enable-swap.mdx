...
- You have an EVM wallet account in the parent organization to receive fees, or you omit `feeReceiverWalletAddress` and let Turnkey generate one. Fees accrue off-chain and are paid out in USDC on Base. A single receiving account covers all swap volume.
...
- **Fee currency and destination.** Fees accrue as Relay app-fee balances for your configured `feeReceiverWalletAddress`, regardless of swap chains or assets. Collect them with [`ACTIVITY_TYPE_CLAIM_SWAP_FEES`](/api-reference/activities/claim-swap-fees). The payout is USDC on Base.
