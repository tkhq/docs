...
- **Field population.** `destinationTxHash` and `providerStatus` are cross-chain only. On same-chain `FAILED`, the settled triple derives directly from the origin transaction: `settledAsset = inputToken`, `settledAmount = inputAmount`, `settlementTxHash = <revert hash>` (nothing moved onchain in a swap sense).
...
- `settledAmount`: raw onchain amount of that asset.
- `settlementTxHash`: the onchain transaction that settled the funds.
