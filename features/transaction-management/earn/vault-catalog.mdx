...
Morpho and Aave are both supported, with an identical API shape. Filter with `provider` (`EARN_PROVIDER_MORPHO` or `EARN_PROVIDER_AAVE`), or omit it to get both. See the [chain and asset support tables](/features/transaction-management/earn#supported-protocols-and-chains).

A few fields read differently per provider:

- **Morpho**: curated ERC-4626 vaults. `name` is the vault's name and `curator` lists its curator(s).
- **Aave**: each entry is an Aave v3 reserve. `vaultAddress` is the reserve's underlying asset address, `name` is the reserve symbol, and `curator` is empty (Aave reserves have no curator).

Reserves that Aave has frozen or paused are excluded from the catalog. If a reserve is frozen or paused after you deploy its wrapper, new deposits fail onchain; withdrawals are unaffected.
