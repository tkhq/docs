...
<Note>
  This walkthrough uses [`@turnkey/http`](https://www.npmjs.com/package/@turnkey/http)'s generic `request` method. Every call below also has a typed method in [`@turnkey/sdk-server`](https://www.npmjs.com/package/@turnkey/sdk-server) (`listEarnVaults`, `earnDeployWrapper`, `earnDeposit`, `earnWithdraw`, `listEarnPositions`, and the status queries); the same flow is available as a runnable SDK example at [examples/defi/with-earn](https://github.com/tkhq/sdk/tree/main/examples/defi/with-earn).
</Note>

**Prerequisites**

- Earn enabled for your organization: a Pay-As-You-Go plan or higher (Pro or higher if you want gas sponsorship)
...
<Steps>
  ...
  <Step title="Find a vault">
    Query the catalog for USDC vaults on Base. The chain comes from the CAIP-19 asset identifier; results are sorted by TVL. This example filters to Morpho; pass `EARN_PROVIDER_AAVE` to list Aave reserves instead, or omit `provider` for both. The rest of the flow is identical for either provider.
