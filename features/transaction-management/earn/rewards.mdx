...
<Note>
  Not every vault has rewards. Rewards depend on whether the vault's curator has created a Merkl campaign, and whether that campaign type supports forwarding through Turnkey's FeeWrapper. Display only what `list_earn_rewards` actually returns for the wallet — do not surface reward rates from vault headline APY.
</Note>
