...
<Note>
  **Aave only:** deploying an Aave wrapper takes a small initial deposit from the deploying wallet and permanently locks it in the reserve, so a newly deployed wrapper shows a small position straight away, about \$1 for stablecoin reserves. See [Aave's deployment docs](https://aave.com/docs/vaults/simple-earn/deploy#define-configuration).
</Note>
