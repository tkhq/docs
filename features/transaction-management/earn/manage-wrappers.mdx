...
A deployed wrapper's code is fixed at deployment and points at the same underlying vault, which cannot change. The wrapper contract contains no function permitting Turnkey or your organization to block, restrict, or reverse a withdrawal as those gates are permanently disabled at deployment. Withdrawal timing may still depend on the underlying vault's available liquidity and network conditions, which Turnkey does not control. A wrapper's fee configuration is fixed in the product; changing it requires deploying a new wrapper.
