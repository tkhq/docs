...
2. Configure Allowed Origins. Only requests from these origins may call the proxy (CORS + origin validation). Each entry must be an exact URL (e.g. `https://myapp.com`). By default all origins are allowed (_). Note: partial wildcard patterns like `https://_.myapp.com` are not currently supported.
