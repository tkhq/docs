...
description: "Learn how to use session profiles to issue sessions with limited scope."
---

## What are session profiles?

Session profiles are resources created by the parent organization that allow sessions to be issued with limited scope. When a session is issued with a session profile, the profile's scope is evaluated on every request made with that session. If the scope evaluates to `false`, the request is denied.
...
#### Scope when approving another activity

When a scoped session approves a pending activity with `APPROVE_ACTIVITY`, the scope is evaluated against **the activity being approved**, not against `APPROVE_ACTIVITY` itself.

So a session scoped to signing:

``` ts
activity.type == 'ACTIVITY_TYPE_SIGN_TRANSACTION_V2'
```

can approve a pending `SIGN_TRANSACTION_V2`, but cannot approve a pending `CREATE_API_KEYS_V2` or `UPDATE_ROOT_QUORUM`. The session's authority follows it into the approval, and is judged on what is actually being authorized.
...
Session profiles are commonly used alongside [MFA policies](../mfa/overview) to create tiered authentication flows. For example, you can require MFA to obtain a session with elevated scope, while allowing a basic session without MFA.
