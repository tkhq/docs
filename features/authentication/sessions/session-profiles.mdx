...
- `scope`: the scope string from the profile
- `session_type`: set to the session profile's name (instead of the default `SESSION_TYPE_READ_WRITE`)

If no `sessionProfileId` is passed, the session is issued as a default read-write session with no scope restrictions.
