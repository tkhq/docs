...
(email OTP + OAuth), then immediately enrolling a new passkey. The new credential is scoped to a
...
1. Ahead of time, you create a session profile scoped to authenticator enrollment only, plus a
   ...
   steps, email OTP and then OAuth or SMS OTP, so that recovery is no weaker than the login it
   replaces.
...
6. The user completes the second factor, OAuth or SMS OTP. That produces a new token, which the
...
```json
// MFA policy for recovery: email OTP + OAuth
mfaPolicy: {
  userId: "<user-id>",
