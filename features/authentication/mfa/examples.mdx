...
mfaPolicy: {
  ...
  order: 0, // Evaluation priority; policies with lower values are evaluated first
}
```
...
sessionProfile: {
  sessionProfileName: 'colossal session',
  scope: "true",   // This session profile can be used for any activity
...
mfaPolicy: {
  condition: "activity.resource == 'AUTH' && activity.params.session_profile_id == '11111111-1111-1111-1111-111111111111'",   // This condition ensures that the MFA requirements only apply when the user is trying to retrieve the "colossal session"
  requiredAuthenticationMethods: [
    {
      any: [
        {
...
sessionProfile: {
  sessionProfileName: 'sms-basic-session',
  scope: "activity.action != 'EXPORT'",
...
sessionProfile: {
  sessionProfileName: 'sms-upgraded-session',
  scope: "activity.action == 'EXPORT'",
  expirationSeconds: 900, // 15 minutes
}
...
sessionProfile: {
  sessionProfileName: 'passkey-login-session',
  scope: "true",
  expirationSeconds: 25200, // 7 hours
}
```
...
mfaPolicy: {
  condition: "activity.resource == 'AUTH' && activity.params.session_profile_id == '11111111-1111-1111-1111-111111111111'",
  requiredAuthenticationMethods: [
    {
      any: [
...
mfaPolicy: {
  condition: "activity.resource == 'AUTH' && activity.params.session_profile_id == '33333333-3333-3333-3333-333333333333'",
  requiredAuthenticationMethods: [
    {
      any: [
        { type: "AUTHENTICATION_TYPE_PASSKEY" },
...
mfaPolicy: {
  condition: "activity.resource == 'AUTH' && activity.params.session_profile_id == '22222222-2222-2222-2222-222222222222'",
  requiredAuthenticationMethods: [
    {
      any: [
        {
...
sessionProfile: {
  sessionProfileName: 'sms-safe-session',
...
sessionProfile: {
  sessionProfileName: 'sms-signing-session',
  scope: "true",
  expirationSeconds: 900, // 15 minutes
}
```
...
In this example, a parent organization enforces MFA on an end-user's sub-organization using a [delegated access user](/features/policies/delegated-access/overview). The delegated access user is controlled by the parent org and has a narrowly scoped policy that only allows it to manage MFA policies.
...
- **Delegated access users**
  - When the end user is locked out, both users must approve the `DeleteMfaPolicy` activity
