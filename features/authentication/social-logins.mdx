...
sequenceDiagram
    ...
    Frontend->>Backend: Send auth_code + redirect_uri + API publicKey
    Backend->>Turnkey: Send auth_code + redirect_uri + nonce=sha256(API publicKey) via `OAuth2Authenticate` call
...
<Steps>
  ...
  <Step title="Send Credentials to Backend">
    The frontend sends the `auth_code`, the `redirect_uri` and the API public key to your backend.
  </Step>
  <Step title="Get OIDC Token">
    The backend exchanges the `auth_code` + `redirect_uri` for an OIDC ID Token using the `OAuth2Authenticate` activity, passing `nonce` set to `sha256(publicKey)` based on the API public key. Turnkey issues this token, not the provider, and embeds the `nonce` in it. This binds the resulting OIDC token to the keypair.
  </Step>
  <Step title="Get or Create a Turnkey Sub-organization">
    The backend gets subOrgIds that are associated with that `oidcToken`. If no sub-organization exists, it creates one using the identity in the token.
  </Step>
