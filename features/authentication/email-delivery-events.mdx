...
<Note>
Email delivery events are stored at the parent organization level. Queries made from a sub-organization will return an error.
</Note>
...
Initialize the client once on your server using your API keys and parent organization ID:
...
<Tabs>
  <Tab title="List email events">
    ```ts
    const { emailEvents } = await client.listEmailEvents({
