...
<Warning>
  The `name` field is a human-readable endpoint name and should be non-empty.
  Event types must be passed in `subscriptions[]`; do not pass a top-level
  `eventTypes` field.
</Warning>
...
<Note>
  An SDK verification helper and a key discovery endpoint are releasing soon to
...
<Note>
  ...
  list](/api-reference/queries/list-supported-assets) and are not supported for
