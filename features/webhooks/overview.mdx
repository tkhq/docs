...
Webhook signatures can be verified with `verifyTurnkeyWebhookSignature` from `@turnkey/crypto`.

Automatic key discovery is not available yet. For now, configure Turnkey's webhook verification key statically. A discovery endpoint will be added later for automatic key rotation.

```bash
TURNKEY_WEBHOOK_KEY_ID=turnkey_webhook_signing_key_001
TURNKEY_WEBHOOK_PUBLIC_KEY=5ebf48061c8e5ca73f30381fbde77c47940bc2149c8ef37eff4faf36c5ba8f9a
```
...
The verifier accepts a list of keys so the same integration can support key rotation. Today, configure the current Turnkey webhook signing key statically. When key discovery ships, prefer fetching and caching verification keys from the discovery endpoint.
