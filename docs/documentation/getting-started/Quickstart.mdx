...
- The keypair you just created is for your parent (root) Turnkey organization. **Any code using an API keypair for the parent organization should only ever be used and stored securely server-side, and never exposed to end-users**.
...
{`\n\n// a provider is required if you want to interact with the live network, i.e. broadcast transactions, fetch gas prices, etc.`}
