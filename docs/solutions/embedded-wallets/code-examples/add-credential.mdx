...
<details>
  <summary>What's the purpose of the <code>rpId</code>?</summary>

The <code>rpId</code> is used in WebAuthn to uniquely identify the server that the passkey is associated with.
The <code>rpId</code> is typically the effective domain of the web application, which is the domain portion of the URL without any subdomains.
For example, if your application is hosted at <code>app.example.com</code>, the <code>rpId</code> would typically be <code>example.com</code>.
