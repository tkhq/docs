...
For an in-depth understanding of the email recovery process at Turnkey, you may refer to our docs on [email recovery](../../../documentation/features/email-recovery).
...
This allows you to use the parent organization's public/private key pair to initialize the email recovery process securely.
...
Next, we'll initialize the `iframeClient` which creates a secure iframe within your application.
The iframe will be used in a later step to complete the email recovery process.

You'll need to add an element to your DOM that will serve as the container for the iframe.
