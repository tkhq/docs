---
title: "Create a User with Email Only"
sidebar_position: 3
description: Create a User with Email Only
slug: /embedded-wallet-workflows/create-user-email
---

import Tabs from "@theme/Tabs";
import TabItem from "@theme/TabItem";
import CodeBlock from "@theme/CodeBlock";
import Parameter from "@site/src/components/parameter";

#### 1. Initialize Turnkey
<CodeBlock language="typescript">
{`import { Turnkey, TurnkeySDKBrowserConfig } from "@turnkey/sdk-browser";`}
{`\n\nconst turnkeyConfig: TurnkeySDKBrowserConfig = {...};`}
{`\nconst turnkey = new Turnkey(turnkeyConfig);`}
</CodeBlock>

#### 2. Configure the SubOrganization for the User

<CodeBlock language="typescript">
{`import { DEFAULT_ETHEREUM_ACCOUNTS } from "@turnkey/sdk-browser;"`}
{`\n\nconst subOrganizationConfig = {
  subOrganizationName: <subOrganizationName>,
  rootUsers: [{
    userName: <userEmail>,
    userEmail: <userEmail>,
    apiKeys: [],
    authenticators: []
  }],
  rootQuorumThreshold: 1,
  wallet: {
    walletName: <walletName>,
    accounts: DEFAULT_ETHEREUM_ACCOUNTS
  }
};
`}
</CodeBlock>

#### 3. Call `createSubOrganization` from your backend

<CodeBlock language="typescript">
{`await turnkey.serverSign("createSubOrganization", [subOrganizationConfig]);`}
</CodeBlock>

This is all that is needed to create a user without any authentication credential other than their email address, in the [login](login-user-email) flow you can see how to then authenticate the user after their `subOrganization` is created.
