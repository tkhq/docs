...
<u>Use the sdk-browser package when you’re building browser-based applications that interact with the Turnkey API.</u> If you are working with React – check out our [`@turnkey/sdk-react`](https://www.npmjs.com/package/@turnkey/sdk-react) package.
...
- [How to instantiate a read-only browser client](#currentusersession)
...
- [How to instantiate a passkey client](#passkeyclient)
...
The `TurnkeyIframeClient` class extends `TurnkeyBrowserClient` and specializes it for use with an iframe-based session. It adds the ability to manage credentials and data exchanges securely through an iframe, allowing for a different form of session persistence and interaction within an embedded environment.

For further information on using the `TurnkeyIframeClient`: 
- [How to instantiate an iFrame client](#iframeclient)
...
const subOrganization = await turnkey.getCurrentSubOrganization(); if (subOrganization) { console.log("Current SubOrganization:", subOrganization); }
```
