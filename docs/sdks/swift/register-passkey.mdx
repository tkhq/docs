...
Create an instance of `PasskeyManager`, providing the [Relying Party Identifier](/passkeys/options#rp) and the [presentation anchor](<https://developer.apple.com/documentation/authenticationservices/asauthorizationcontrollerpresentationcontextproviding/presentationanchor(for:)>).
...
sub-organization using TurnkeyClient from the TurnkeySDK.

<Steps>

<Step>Initialize TurnkeyClient with Proxy</Step>

When handling the completion of passkey registration, set up the TurnkeyClient with a proxy server URL using the ProxyMiddleware.
...
- The middleware adds an `X-Forwarded-For` header to each request, which contains the original request URL.
- Ensure that the response from your backend matches exactly what is expected from Turnkey’s backend.

:::

<details>
<summary>Example Node.js Proxy Server</summary>
