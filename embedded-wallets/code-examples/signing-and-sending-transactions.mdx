...
Traditionally, sending transactions onchain has been hard:
...
Turnkey makes this as simple as a couple of API calls: we seamlessly handle gassing and our battle-tested logic ensures inclusion even in the most adverse network conditions. You and your users never have to worry about touching gas tokens or stuck transactions.
...
Through a single endpoint, you can easily toggle between standard eip-1559 transactions and gas sponsored transactions. To enable sponsorship simply set `sponsor: true`.

### Construction and Broadcast

Regardless of whether you set `sponsor: true`, Turnkey handles transaction construction and broadcast. Turnkey auto-fills any fields you omit from the request, including nonce, data, value, and gas fields.
