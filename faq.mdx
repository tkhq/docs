...
<AccordionGroup>
  ...
  <Accordion title="Do you have any rate limits in place in your public API?">
    Our public API currently limits users to a maximum of 60 RPS (Requests Per Second), depending on their account plan. Specific headers are returned to indicate current quota:

    - `ratelimit-limit`: indicates the total quota
    ...
    This limit is on a **per IP address** basis: if you have multiple servers making requests to the turnkey API under a different IP address, each server is subject to the following plan based limits:

    - **Free customers:** 60 RPM per org family, no separate per-sub-org limit
    - **Pay as you gocustomers**: 60 RPM per org family, no separate per-sub-org limit
    - **Pro customers**: 180 RPM per org family, no separate per-sub-org limit
    - **Enterprise customers**: 3600 RPM per org family, 600 RPM per sub org
