...
We have example repositories that demonstrate how to construct and sign Tempo transactions:
...
- **Raw calldata inspection**: Inspect ABI-encoded arguments in call input data using slicing (e.g. `tempo.tx.calls[0].input[34..74]`, which extracts a 20-byte Ethereum-style address argument from ABI-encoded calldata after the 4-byte selector and 12 bytes of left-padding) and verify that extracted address with [`IsGuaranteedWalletAccountInWallet`](/concepts/policies/language#built-in-isguaranteedwalletaccountinwallet)
