...
- [**Catena**](https://www.turnkey.com/customers) built agentic treasury and banking infrastructure on Turnkey, giving AI agents their own accounts, payments, and treasury management. Policy guardrails are enforced at the enclave level, including tamper-proof spend limits, multi-party approvals, and agent-to-agent commerce with built-in identity verification.
...
| Payments restricted to approved recipients | Policies enforce destination allowlists per wallet, so agents can only send to known addresses |
| Transactions must complete reliably | [Transaction lifecycle management](/features/transaction-management) handles broadcasting, confirmation tracking, and retries in a single API call |
| End users and agents should not manage gas | [Sponsored transactions](/features/transaction-management/broadcasting) cover gas fees so neither agents nor recipients need native tokens |
| Agent needs to know available funds before paying | [Real-time balances](/features/balances) let agents check what they can spend before initiating a payment |
| High-value payments need human approval | Consensus policies require a human co-signer above configurable thresholds |
| Must support protocol-level payment flows | The same wallet and signing infrastructure works for direct transfers, x402 responses, and MPP authorization |
...
With gas sponsorship enabled, the agent doesn't need to hold ETH to pay for transaction fees. Turnkey covers gas costs so the agent can transact using only its USDC balance:
