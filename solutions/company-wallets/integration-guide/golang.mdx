...
client delegates that step to any value implementing the `Stamper` interface:
...
Because this is the only contract the client depends on, stamping is a replaceable boundary.
`NewAPIKeyStamper` is the standard server-side implementation for API keys, but you can supply any
implementation to move the signing decision to the place that best matches your security boundary:

- An **API key stamper** (the built-in default) for ordinary backend service calls.
- A **policy or KMS stamper** that routes signing through your own key-management or approval layer.
- A **WebAuthn-backed stamper** when request authorization should come from a passkey or
  authenticator ceremony. See [Example: WebAuthn stamper](#example-webauthn-stamper).
...
The skeleton below shows the shape every implementation shares — take the body, produce a
signature with whatever mechanism you control, and package it as a `Stamp`:
