...
async function handleSVMSwap() {
  ...
  const txBuffer = Buffer.from(
    getPriceResponse.transactionRequest.data,
    "base64",
  );
...
async function getStatus() {
  ...
  while (true) {
    ...
    if (getStatusResponse.status == "DONE") {
      if (fromToken === "ETH") {
        console.log(
          "SOL Receiving Transaction:" + getStatusResponse.receiving.txHash,
        );
      } else if (fromToken === "SOL") {
        console.log(
          "ETH Receiving Transaction:" + getStatusResponse.receiving.txHash,
        );
      }
      break;
    }
