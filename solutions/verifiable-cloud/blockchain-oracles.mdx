...
| **Basis of trust** | Independent operators and a reporting protocol establish agreement on the reported value. | Hardware attestation establishes application identity; reviewed code defines how inputs become outputs. |
| **Verification** | In a quorum-reporting design, the receiving contract verifies report signatures and the required operator threshold. | Third parties can verify the application's Boot Proof and code identity. App Proofs can bind individual outputs to that application; on-chain verification requires an explicit integration. |
...
<Note>
**Private inputs, verifiable conditions**

A lender needs to know whether a borrower has enough eligible receivables to support a requested draw, without publishing its customer list, invoice amounts, or outstanding debt.

A TVC oracle can fetch records from the borrower's accounting system and loan servicer, exclude overdue or disputed invoices, apply the facility's advance rates and concentration limits, and calculate the available borrowing capacity inside the enclave. It publishes only whether the requested draw satisfies those rules, tied to the request and observation time.

An App Proof can bind the decision to the reviewed calculation. With output verification integrated into the receiving contract, the facility can authorize the draw while keeping the underlying financial records off-chain and confidential.
</Note>
