...
Turnkey's public API has an open CORS policy, so your SDK can POST signed requests straight to `api.turnkey.com`. Routing them through your backend instead gives your platform a single egress point for every Turnkey call. At that chokepoint you can validate requests before forwarding them, apply rate limits per merchant or per user, persist activity results such as new wallet addresses, and feed your own monitoring.
