...
If you are using Turnkey's Auth Proxy, ensure it is enabled in the Turnkey Dashboard under the **Embedded Wallets → Configuration** section. You can also check the [Getting Started](/solutions/embedded-wallets/integration-guide/react/getting-started) guide for more details on setting up the Auth Poxy.
...
To resolve this issue, you need to enable the Auth Proxy in the Turnkey Dashboard under the **Embedded Wallets → Configuration** section. Once enabled, you will receive an `authProxyConfigId` that you can use in your `TurnkeyProvider` config. Pass this `authProxyConfigId` to the `TurnkeyProvider` config to enable the authentication flow.
...
For Tailwind CSS v3, you need to import the Turnkey styles in your main CSS file (e.g., `styles/globals.css`) by pointing the import to `@turnkey/react-wallet-kit/dist/styles.css` file:
...
### 8. Module not found. The extension in the request is mandatory for it to be fully specified (Create React App)

<img
  src="/images/sdks/img/react/fully-specified.png"
  alt="Create React App error"
/>
...
<Steps>
  <Step title="Eject from Create React App (CRA)">
  Ejecting from CRA will give you full control over the Webpack configuration, allowing you to modify it to remove the need for fully specified imports.
  ```bash
  npm run eject
  ```
  Note that this action is irreversible, so make sure to commit your changes before ejecting. 
  </Step>
  <Step title="Modify the Webpack configuration">

After ejecting, open the `config/webpack.config.js` file and locate the `resolve` section. Add the following line to the `resolve` object under the rules for `/\.(js|mjs)$/` files:
...
// ... other configurations
{
  test: /\.(js|mjs)$/,  // Search for this line
  exclude: /@babel(?:\/|\\{1,2})runtime/,
  loader: require.resolve('babel-loader'),

  resolve: {
    fullySpecified: false,  // Add this line
  },
...
This will disable the requirement for fully specified imports in your project.

  </Step>
</Steps>

Read more about fully specified imports in the [Webpack documentation](https://webpack.js.org/configuration/module/#resolvefullyspecified).

### 9. Client is not initialized error when using Turnkey functions

<img
  src="/images/sdks/img/react/client-not-initialized.png"
  alt="Client is not initialized error"
/>
