...
We'll use `@turnkey/react-wallet-kit` for browser-based authentication and wallet creation, and Turnkey's `signMessage` to authorize card funding — all without the user's keys ever leaving Turnkey's secure infrastructure.

A working example can be found [here](https://github.com/tkhq/sdk/tree/main/examples/with-rain).
