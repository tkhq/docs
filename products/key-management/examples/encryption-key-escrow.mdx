...
Recovery Key Storage separates risk between two independent parties. Your infrastructure holds encrypted data that Turnkey never sees, and Turnkey's secure enclave holds the encryption key that your infrastructure never touches. Both are required to access plaintext data, creating a **2-of-2 security model** where neither party alone can compromise the data.
...
Turnkey supports multiple authentication methods for key export: API keys, [passkeys](/authentication/passkeys/introduction), [social logins](/authentication/social-logins), [email](/authentication/email) and [SMS OTP](/authentication/sms), allowing you to match your security requirements.
