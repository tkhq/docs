...
<Steps>
  ...
  <Step title="Active session">
    Sign transactions with any wallet—zero network latency, zero Turnkey API calls. Scale to hundreds of wallets without performance degradation.
  </Step>
...
- Transaction signing: <5ms (pure local operation)
...
<Steps>
  ...
  <Step title="Decrypt and use locally">
    Decrypt your stored bundles and use them—all locally, with no further Turnkey calls:
