...
The iframe typically exposes to the user in plaintext, though this is not a requirement. The target encryption key (TEK) can be any arbitrary P256 key, and it is possible (using [PBKDF2](https://en.wikipedia.org/wiki/PBKDF2)) for this target key to represent a custom passphrase provided by the user. This can be implemented manually today, and Turnkey expects to offer it as a stock feature of the iframe shortly alongside the SDK option of saving to filesystem or secure cloud.
