...
<ResponseField name="windowDurationMinutes" type="number" required={true}>
  The window duration (in minutes) for the organization or sub-organization.
</ResponseField>
<ResponseField name="windowLimitUsd" type="string" required={true}>
  The window limit (in USD) for the organization or sub-organization.
</ResponseField>
<ResponseField name="usageUsd" type="string" required={true}>
  The total gas usage (in USD) of all sponsored transactions processed over the
  last `window_duration_minutes`
</ResponseField>
