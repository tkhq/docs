---
title: "Get Earn withdraw status"
...
<EndpointPath type="query" path="get_earn_withdraw_status" />
...
<ParamField body="withdrawRequestId" type="string" required={true}>
...
```bash title="cURL"
curl --request POST \
  --url https://api.turnkey.com/public/v1/query/get_earn_withdraw_status \
...
const response = await turnkeyClient.apiClient().getEarnWithdrawStatus({
  organizationId: "<string> (Unique identifier for a given Organization.)",
  withdrawRequestId: "<string> (The withdraw_request_id returned by EarnWithdraw.)"
});
```
