...
Optional blockhash used for signing. For non-sponsored transactions, it must match the blockhash in `unsignedTransaction` or replace its all-zero placeholder. If the placeholder is present and this field is omitted, Turnkey fetches a blockhash before policy evaluation. Filling the placeholder requires an unsigned transaction and all required signers. Sponsored transactions use this value or fetch one during execution. Turnkey does not refresh expired blockhashes.
</NestedParam>
