...
<EndpointPath type="query" path="get_earn_deposit_status" />
...
<ParamField body="depositRequestId" type="string" required={true}>
...
```bash title="cURL"
curl --request POST \
  --url https://api.turnkey.com/public/v1/query/get_earn_deposit_status \
...
const response = await turnkeyClient.apiClient().getEarnDepositStatus({
  organizationId: "<string> (Unique identifier for a given Organization.)",
  depositRequestId: "<string> (The deposit_request_id returned by EarnDeposit.)"
});
```
