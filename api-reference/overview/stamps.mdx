...
<Steps>
   ...
   <Step>
      Create a JSON-encoded stamp:
         ...
         * `publicKey`: the public key of the API key. Turnkey supports multiple API key curves, including `API_KEY_CURVE_P256`, `API_KEY_CURVE_SECP256K1`, and `API_KEY_CURVE_ED25519`. The `publicKey` value is the compressed (for P-256 and SECP256K1) or raw (for Ed25519) public key, hex encoded.
         * `signature`: the signature produced by the API key over the JSON-encoded POST body, hex encoded.
         * `scheme`: the signature scheme used to sign the request. For example, P-256 API keys use `SIGNATURE_SCHEME_TK_API_P256`. The scheme value identifies how the server should verify the signature; if you use a non-P-256 curve, refer to the API key stamper SDK for the matching scheme value.
   </Step>
   <Step>
