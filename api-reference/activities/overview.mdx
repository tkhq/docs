...
- **Approval expiration:** Activities do not expire. However, when an activity is submitted, the requester's submission counts as the first approval and starts a 24-hour window. If the activity is not resolved within that window, existing approvals expire and must be re-submitted. This applies both while the activity is in `ACTIVITY_STATUS_CONSENSUS_NEEDED` awaiting votes, and in `ACTIVITY_STATUS_AUTHENTICATORS_NEEDED` awaiting [MFA](/features/authentication/mfa/satisfying-mfa) factors.
...
- **Request freshness:** `timestampMs` must be within the last hour, and no more than five minutes in the future. Older requests are rejected with `activity timestamp is not current` rather than returning the original activity, so use `get_activity` to check on anything submitted more than an hour ago. This applies to every submission, including approvals and rejections.
