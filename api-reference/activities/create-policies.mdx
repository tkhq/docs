  ...
  <Expandable title="details">
    <ParamField body="policies" type="array" required={true} path="parameters.policies">
      ...
      <Expandable title="item details">
        ...
        <NestedParam parentKey="parameters.policies" childKey="notes" type="string" required={true} default="">
        Notes for a Policy.
        </NestedParam>
        
        <NestedParam parentKey="parameters.policies" childKey="time" type="string" required={false} default="">
...
const response = await turnkeyClient.apiClient().createPolicies({
  policies: [{ // An array of policy intents to be created.,
    ...
    notes: "<string> (Notes for a Policy.)",
    time: "<string> (The time expression that triggers the Effect)",
  }]
});
```
