...
Enum options: `ACTIVITY_TYPE_SOL_SEND_TRANSACTION_V2`
</ParamField>
...
<ParamField body="parameters" type="object" required={true} path="parameters">
  ...
  <Expandable title="details">
    ...
    <ParamField body="signWiths" type="array" required={true} path="parameters.signWiths">
      <p>Ordered Solana signer addresses Turnkey signs with. Between 1 and 16 signers for legacy/V0, or up to 12 for V1 (11 when sponsored). For sponsored transactions this must list every required signer of the transaction in transaction order.</p>
...
const response = await turnkeyClient.apiClient().solSendTransaction({
  unsignedTransaction: "<string> (Hex-encoded serialized unsigned Solana transaction in full wire format. Legacy/V0 transactions allow 1232 bytes. V1 allows 4096 bytes with up to 12 trailing 64-byte signature slots, including the paymaster for sponsored transactions. Fill unsigned slots with zeroes.)",
  signWiths: ["<string>"] // Ordered Solana signer addresses Turnkey signs with. Between 1 and 16 signers for legacy/V0, or up to 12 for V1 (11 when sponsored). For sponsored transactions this must list every required signer of the transaction in transaction order.,
  ...
  recentBlockhash: "<string> (Optional blockhash used for signing. For non-sponsored transactions, it must match the blockhash in `unsignedTransaction` or replace its all-zero placeholder. If the placeholder is present and this field is omitted, Turnkey fetches a blockhash before policy evaluation. Filling the placeholder requires an unsigned transaction and all required signers. Sponsored transactions use this value or fetch one during execution. Turnkey does not refresh expired blockhashes.)"
});
```
