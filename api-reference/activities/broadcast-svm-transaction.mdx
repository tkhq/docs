...
<ParamField body="parameters" type="object" required={true} path="parameters">
  ...
  <Expandable title="details">
    ...
    <ParamField body="signWiths" type="array" required={true} path="parameters.signWiths">
      <p>Ordered Solana signer addresses Turnkey signs with. Between 1 and 16 signers. For sponsored transactions this must list every required signer of the transaction in transaction order.</p>
    ...
    Optional blockhash to fix the value used for signing. For non-sponsored sends, it must match a nonzero embedded blockhash; otherwise Turnkey inserts it into the all-zero placeholder. If neither value is supplied, Turnkey fetches a blockhash before policy evaluation. Filling the placeholder requires an unsigned transaction and all required signers. Sponsored sends use this value if supplied, or fetch a blockhash during execution. Supplied blockhashes are not refreshed when expired. See [Blockhash selection and expiration](/features/networks/solana-transactions#blockhash-selection-and-expiration).
    </NestedParam>
  </Expandable>
</ParamField>
...
const response = await turnkeyClient.apiClient().solSendTransaction({
  ...
  recentBlockhash: "<string> (Optional blockhash to fix the value used for signing. For non-sponsored sends, it must match a nonzero embedded blockhash; otherwise Turnkey inserts it into the all-zero placeholder. If neither value is supplied, Turnkey fetches a blockhash before policy evaluation. Filling the placeholder requires an unsigned transaction and all required signers. Sponsored sends use this value if supplied, or fetch a blockhash during execution. Supplied blockhashes are not refreshed when expired. See [Blockhash selection and expiration](/features/networks/solana-transactions#blockhash-selection-and-expiration).)"
});
```
