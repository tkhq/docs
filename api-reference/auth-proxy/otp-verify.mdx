...
<ResponseField name="verificationToken" type="string" required={true}>
  Verification Token containing a unique id, expiry, verification type, contact
  signed by Turnkey's enclaves. Verification status of a user is updated when
  the token is consumed (in OTP_LOGIN requests)
</ResponseField>
