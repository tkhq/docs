...
```json
{
  ...
  "quotedOutputAmount": "<QUOTED_OUTPUT>",
...
_(Field names and JSON casing `To Confirm`.)_
...
- `provider`: the provider that executed the swap. Currently `zero_x` in V1.
- `inputToken`, `outputToken`, `inputAmount`: echoed from the execute intent.
- `quotedOutputAmount`, `minOutputAmount`: from the quote. Always present. `minOutputAmount` is a guaranteed floor.
...
- `updatedAt`: last observed state change.
